...
enum sp_return sp_get_port_by_name(const char *portname, struct sp_port **port_ptr)
{
    ...
    debug_on = getenv("LIBSERIALPORT_DEBUG");
                                        ...
                                        size_t count, unsigned int timeout_ms)
{
    ...
    /* Start write. */
    if (WriteFile(port->hdl, buf, (int)count, NULL, &port->write_ovl)) {
        DEBUG("Write completed immediately");
        RETURN_INT((int)count);
    } else if (GetLastError() == ERROR_IO_PENDING) {
                                           ...
                                           const void *buf, size_t count)
{
    ...
    buf_bytes = min(port->write_buf_size, (int)count);
                                       ...
                                       size_t count, unsigned int timeout_ms)
{
    ...
    /* Start read. */
    if (ReadFile(port->hdl, buf, (int)count, NULL, &port->read_ovl)) {
                                            ...
                                            size_t count, unsigned int timeout_ms)
{
    ...
    while (bytes_read == 0) {
        /* Start read. */
        if (ReadFile(port->hdl, buf, (int)count, &bytes_read, &port->read_ovl)) {
            DEBUG("Read completed immediately");
        } else if (GetLastError() == ERROR_IO_PENDING) {
            DEBUG("Waiting for read to complete");
            if (GetOverlappedResult(port->hdl, &port->read_ovl, &bytes_read, TRUE) == 0)
                                          ...
                                          size_t count)
{
    ...
    /* Do read. */
    if (ReadFile(port->hdl, buf, (int)count, NULL, &port->read_ovl) == 0)
        if (GetLastError() != ERROR_IO_PENDING)
            RETURN_FAIL("ReadFile() failed");
